...
import { ShieldCheck, Menu, X, Search, Bell, Sparkles, LogOut, User } from 'lucide-react';
...
export function Navbar() {
  ...
  const { user, role, clearUser } = useStore();
          ...
          <div className="hidden md:flex items-center space-x-4">
            {user ? (
               <div className="flex items-center gap-4">
                 ...
                 <Link to={role === 'admin' ? "/admin" : "/dashboard"} className="flex items-center gap-2 text-sm font-medium text-slate-700 hover:text-primary-900 transition ml-2">
                   <User className="w-5 h-5"/> {user.name}
                 </Link>
                 <Button variant="ghost" className="text-slate-600 font-medium hover:bg-slate-50 flex gap-2 items-center" onClick={handleLogout}>
        ...
        {isOpen && (
          ...
          >
            <div className="px-4 pt-2 pb-6 flex flex-col space-y-4">
               ...
               {user ? (
                 <>
                   <Link to={role === 'admin' ? "/admin" : "/dashboard"} onClick={() => setIsOpen(false)} className="text-primary-900 font-medium">Dashboard</Link>
                   <button onClick={() => { handleLogout(); setIsOpen(false); }} className="text-left text-red-600 font-medium">Logout</button>
                 </>
               ) : (
                 <>
