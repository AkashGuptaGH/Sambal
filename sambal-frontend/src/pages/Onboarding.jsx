...
const SUPPORTED_CITIES = ["Mumbai", "Delhi", "Bangalore", "Bengaluru", "Chennai", "Hyderabad", "Kolkata", "Pune", "Ahmedabad", "Jaipur", "Gurugram", "Noida", "Demo: Storm Zone", "Other (Unsupported City)"];

export default function Onboarding() {
  const [currentStep, setCurrentStep] = useState(0);
  const [formData, setFormData] = useState({ platform: '', city: 'Bengaluru', zone: '', earnings: '', persona: 'food_delivery' });
       ...
       try {
          ...
          const response = await fetch('http://127.0.0.1:8000/api/onboard/risk-profile', {
             ...
             body: JSON.stringify({
                ...
                // Derive zone number from zone string length (varies input → varies score)
                delivery_zone: Math.min(5, Math.max(1, Math.ceil((formData.zone?.length || 10) / 4))),
                persona: formData.persona || "food_delivery",
                avg_daily_earn: parseFloat(formData.earnings) || 1200,
                // Estimate hours: lower earnings → fewer hours; higher → more hours
                weekly_hours: Math.min(70, Math.max(20, Math.round((parseFloat(formData.earnings) || 1200) / 20))),
          ...
          setRiskResult({ risk_score: 1.1, model_version: 'ERR-FALLBACK', zone_multiplier: 1.0 });
          setCurrentStep(3);
       } finally {
          setIsProcessing(false);
       }
    } else if (currentStep === 3) {
       navigate('/premium-breakdown', { state: { riskResult, formData } });
    } else if (currentStep < 3) {
               ...
               {currentStep === 0 && (
                 <motion.div key="step-0" variants={fadeVariants} initial="hidden" animate="visible" exit="exit" transition={{ duration: 0.3 }} className="space-y-6 flex-1">
                   ...
                   <div className="grid grid-cols-2 gap-4 mt-6">
                      {['Swiggy', 'Zomato', 'Blinkit', 'Zepto', 'Amazon', 'Other'].map((p, index) => (
                         <div 
                           key={index} 
                           onClick={() => {
                             const pMap = { Swiggy: 'food_delivery', Zomato: 'food_delivery', Blinkit: 'grocery', Zepto: 'grocery', Amazon: 'logistics', Other: 'ride_hailing' };
                             setFormData({...formData, platform: p, persona: pMap[p] || 'food_delivery'});
                           }}
                   ...
                   <div className="mt-8 space-y-5">
                      <div className="space-y-2 relative">
                        ...
                        <select 
                          ...
                          className={`w-full h-12 rounded-md border border-slate-200 px-3 focus:outline-none focus:border-primary-900 focus:ring-1 focus:ring-primary-900 text-lg font-bold appearance-none bg-white ${
                            formData.city && !isCitySupported ? 'bg-amber-50/10' : ''
                          }`}
                        >
                          <option value="" disabled>Select City</option>
                          {['Mumbai', 'Delhi', 'Bangalore', 'Chennai', 'Hyderabad', 'Kolkata', 'Pune', 'Ahmedabad', 'Jaipur', 'Gurugram', 'Noida', 'Demo: Storm Zone', 'Other (Unsupported City)'].map(c => (
                            <option key={c} value={c}>{c}</option>
                          ))}
                        </select>
                        <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pt-5 pr-4 text-slate-400">
                          <svg className="w-5 h-5 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
                        </div>
                      </div>
